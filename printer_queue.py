...
def optimize_printing(print_jobs: List[Dict], constraints: Dict) -> Dict:
    ...
    print_order = []
    total_time = 0
    printing_jobs = [PrintJob(**job) for job in print_jobs]
    constraints = PrinterConstraints(**constraints)
    printing_jobs.sort(key=lambda printing_jobs: printing_jobs.priority)
    ...
    while printing_jobs:
        ...
        for job in printing_jobs:
            if len(current_group) < constraints.max_items and current_volume + job.volume <= constraints.max_volume:
                current_group.append(job)
                current_volume += job.volume

        for job in current_group:
            printing_jobs.remove(job)
